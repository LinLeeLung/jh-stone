...
const h3 = (html.match(/<h3[^>]*>([\s\S]*?)<\/h3>/i)?.[1] || "").replace(/<[^>]+>/g, " ").trim();
const og = (html.match(/property="og:image"[^>]*content="([^"]+)"/i)?.[1] || "").trim();

const imgs = [...html.matchAll(/<img[^>]+src="([^"]+)"[^>]*>/gi)].map((m) => m[1]);
const texts = [...html.matchAll(/<span[^>]*>([^<]+)<\/span>/gi)].map((m) => m[1].trim()).filter(Boolean);
