...
const categoryLinks = [...html.matchAll(/<a[^>]+href="([^"]*product\.php\?[^"#]+)"[^>]*>([^<]*)<\/a>/gi)]
  .map((m) => ({ href: m[1], text: m[2].trim() }))
  .filter((item) => item.href.includes("pid="));

const detailLinks = [...html.matchAll(/<a[^>]+href="([^"]*product_detail\.php\?id=\d+)"[^>]*>([^<]*)<\/a>/gi)]
  .map((m) => ({ href: m[1], text: m[2].trim() }));
...
const cardImageNearDetail = [...html.matchAll(/<img[^>]+src="([^"]+)"[^>]*>[\s\S]{0,400}<a[^>]+href="([^"]*product_detail\.php\?id=\d+)"/gi)]
