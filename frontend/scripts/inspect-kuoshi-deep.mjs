...
for (const key of keys) {
  ...
  if (index >= 0) {
    console.log(
      html.slice(Math.max(0, index - 260), Math.min(html.length, index + 360)),
    );
  }
}

const pidLinks = [...html.matchAll(/href=["']([^"']*pid=\d+[^"']*)["']/gi)].map(
  (m) => m[1],
);
console.log("PID_LINKS", pidLinks.length);
console.log(pidLinks.slice(0, 50));

const onclicks = [...html.matchAll(/onclick=["']([^"']+)["']/gi)].map(
  (m) => m[1],
);
const interestingOnclick = onclicks.filter((v) =>
  /pid|cat|product|page|fill|tab|class|show|ajax/i.test(v),
);
...
const hints = scripts
  .flatMap((s) =>
    [
      ...s.matchAll(
        /(product\.php\?[^'"\s]+|product_detail\.php\?id=\d+|pid\s*=\s*\d+|cid\s*=\s*\d+|cat\w*\s*=\s*\d+|page\s*=\s*\d+)/gi,
      ),
    ].map((m) => m[1]),
  )
  .filter(Boolean);
