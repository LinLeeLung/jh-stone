<template>
  <div class="bg-white p-4 rounded-lg shadow-md w-full min-w-0 max-w-[700px]">
    <div class="flex flex-wrap gap-2 mb-2 items-center text-sm">
      <h2 class="font-semibold text-gray-700">美背</h2>
    ...
    <div
      class="text-sm grid grid-cols-2 gap-2 mb-1 text-center font-semibold text-gray-600"
    >
      <span>長度</span>
      <span>高度</span>
    </div>
  ...
  setup(props, { emit }) {
    ...
    const calculate = () => {
      ...
      const { cmValue, calcSteps, area, calcSteps2, frontEdgeLength } =
        calcOneSide(f.length, f.depth, f.limit);
